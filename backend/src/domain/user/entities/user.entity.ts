...
export class User extends BaseEntity {
    ...
    @Column({ type: 'enum', enum: LoginProvider, name:'login_provider', nullable: false })
    loginProvider: LoginProvider;

    @Column({ type: 'enum', enum: JoinPath, name: 'join_path', nullable: false })
    joinPath: JoinPath;

    @Column({ type: 'enum', enum: UserStatus, name: 'user_status', nullable: false })
    userStatus: UserStatus;

    @Column({ type: 'timestamp', name: 'last_login_date', nullable: false })
    lastLoginDate: Date;

    @Column({ type: 'timestamp', name: 'last_password_change_date', nullable: false })
    lastPasswordChangeDate: Date;

    constructor(email: string, password: string, name: string, loginProvider: LoginProvider, joinPath: JoinPath, userStatus: UserStatus) {
        super();
        this.email = email;
